...
var test_vertix = [{"node" : 1, "vertex": [2, 3, 4]}, {"node": 2, "vertex": [4, 3, 7, 8]}]
...
function develop_boolean_matrix(all_nodes, connections) {
  ...
  for (let col_count = 0; col_count <= all_nodes.length; col_count++) {
    ...
    for (let row_count = 0; row_count <= all_nodes.length; row_count++) {
     col_count == 0 || row_count == 0 ? current_row[row_count] = `${Math.max(row_count, col_count)}  ` : current_row[row_count] = false
    }
...
function update_graph(key, values, connections) {
  for (value of values) {
    ...
    connections[key][value] = true
    connections[value][key] = true
  }
}
